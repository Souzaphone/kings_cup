...
TIERS_DIR = Path(__file__).parent / "config" / "tiers"
...
def load_blocked_commands():
    ...
    1. Legacy: flat JSON array of rules (backwards compatible)
    2. Tiered: {"tier": "normal", "additional_rules": [...]}
       Loads preset from config/tiers/{tier}.json, then
       appends additional_rules on top.
    """
    ...
    # Legacy format: flat array of rules
    if isinstance(config, list):
        return config

    # Tiered format: load preset + merge additional rules
    tier = config.get("tier", "normal")
    additional = config.get("additional_rules", [])
    ...
    try:
        with open(tier_path, "r") as f:
            tier_rules = json.load(f)
    except (json.JSONDecodeError, ValueError):
        tier_rules = []

    # Filter out comment-only entries from tier presets
    tier_rules = [r for r in tier_rules if r.get("pattern")]

    return tier_rules + additional
...
def check_blocked_commands(command):
    ...
    # git commit/tag are safe — don't match patterns in message text
    if re.match(r"\s*git\s+(commit|tag)\b", normalized):
        return None, None
